...
import { deferredPromise, iterToAsyncGenerator, noop, pipe } from './utils.js'
...
export class AsyncGeneratorObservable {
    ...
    subscribe(observer) {
        const it = iterToStoppable(this.#gen())
        ;(async function () {
            for await (const _ of tap(observer)(iterToAsyncGenerator(it))()) {
...
function iterToStoppable(iterator) {
    const it = iterator
    let stopped = false
    let defer = null
    return {
